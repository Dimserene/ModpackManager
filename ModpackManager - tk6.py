...
class ModpackManagerApp:
    def __init__(self, root):
        ...
        # Check for Lovely Injector
        self.check_lovely_injector()

        self.create_widgets()
        self.update_installed_info()  # Initial update

    def check_lovely_injector(self):
        if platform.system() == "Windows":
            game_directory = r"C:\Program Files (x86)\Steam\steamapps\common\Balatro"
        else:
            game_directory = os.path.expanduser("~/.steam/steam/steamapps/common/Balatro")

        version_dll_path = os.path.join(game_directory, "version.dll")

        if not os.path.exists(version_dll_path):
            response = messagebox.askyesno(
                "Lovely Injector Not Installed",
                "Lovely injector not installed. Do you want to install it?\n\nTo ensure that the installation proceeds smoothly, please disable any antivirus software temporarily."
            )
            if response:
                self.download_and_extract_lovely_injector(game_directory)
            else:
                return

    def download_and_extract_lovely_injector(self, target_directory):
        url = "https://github.com/ethangreen-dev/lovely-injector/releases/latest/download/lovely-x86_64-pc-windows-msvc.zip"
        zip_file_path = os.path.join(target_directory, "lovely-injector.zip")
