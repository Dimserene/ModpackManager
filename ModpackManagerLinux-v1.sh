...
MODS_DIR="$HOME/.local/share/Balatro/Mods"
MODPACK_DOWNLOAD_DIR="$HOME/Downloads/Modpack"
...
DIMSERENE_MODPACK_NAME="Dimserene's Modpack"
FINE_TUNED_MODPACK_NAME="Fine-tuned Pack"
VANILLA_PLUS_MODPACK_NAME="Vanilla+ Pack"
...
display_menu() {
  ...
  echo "5. Install Lovely Injector"
  echo "6. Exit"
  echo ""
}
...
get_installed_version() {
  local version_file="$MODS_DIR/CurrentVersion.txt"
...
get_installed_pack_name() {
  local modpack_util="$MODS_DIR/ModpackUtil.lua"
...
check_versions() {
  ...
  if [ "$installed_version" != "$latest_version" ]; then
    ...
    echo ""
    echo "Update available for $installed_pack_name!"
  else
...
download_modpack() {
  ...
  if [ -d "$MODPACK_DOWNLOAD_DIR" ]; then
    echo "Modpack directory already exists. Removing the old one."
    rm -rf "$MODPACK_DOWNLOAD_DIR"
  fi
  echo "Cloning modpack $MODPACK_NAME from $MODPACK_URL..."
  git clone --recurse-submodules --remote-submodules "$MODPACK_URL" "$MODPACK_DOWNLOAD_DIR"
  if [ $? -eq 0 ]; then
    echo "$MODPACK_NAME downloaded successfully to $MODPACK_DOWNLOAD_DIR."
  else
    echo "Failed to download $MODPACK_NAME."
  fi
}
...
install_modpack() {
  ...
  echo "Installing $MODPACK_NAME..."
  if [ ! -d "$MODS_DIR" ]; then
    echo "Mods directory does not exist. Creating it..."
    mkdir -p "$MODS_DIR"
  fi
  if [ -d "$MODPACK_DOWNLOAD_DIR/Mods" ]; then
    cp -r "$MODPACK_DOWNLOAD_DIR/Mods/"* "$MODS_DIR"
    if [ $? -eq 0 ]; then
      echo "$MODPACK_NAME installed successfully to $MODS_DIR."
    else
      echo "Failed to install $MODPACK_NAME."
    fi
  else
