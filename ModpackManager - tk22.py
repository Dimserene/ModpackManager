    ...
    def run(self):
        try:
            ...
            if not self.process.waitForFinished(-1):  # Wait indefinitely until finished
                self.finished.emit(False, f"Download failed: {self.process.errorString()}")
            else:
...
############################################################
    ...
    def create_widgets(self):
        ...
        default_modpack = self.settings.get("default_modpack", "Dimserene's Modpack")  # Get default modpack or empty string if not set
        ...
        default_modpack = self.settings.get("default_modpack", "")
        if default_modpack in [self.modpack_var.itemText(i) for i in range(self.modpack_var.count())]:
