<<<<<<< HEAD
import platform, os, re, shutil, requests, webbrowser, subprocess, zipfile, stat, json, git, time
...
if current_platform == "Linux":
    DEFAULT_SETTINGS["game_directory"] = os.path.expanduser("~/.steam/steam/steamapps/common/Balatro")
    DEFAULT_SETTINGS["mods_directory"] = os.path.expanduser("~/.steam/steam/steamapps/compatdata/2379780/pfx/drive_c/users/steamuser/AppData/Roaming/Balatro/Mods")
elif current_platform == "Darwin":  # MacOS
    DEFAULT_SETTINGS["game_directory"] = os.path.expanduser("~/Library/Application Support/Steam/SteamApps/common/Balatro")
    DEFAULT_SETTINGS["mods_directory"] = os.path.expanduser("~/Library/Application Support/Balatro/Mods")
...
############################################################
    ...
    def get_save_paths(self):
        """Return the appropriate paths based on the platform."""
        if platform.system() == "Linux":
            save_file_path = os.path.expanduser("~/.steam/steam/steamapps/compatdata/2379780/pfx/drive_c/users/steamuser/AppData/Roaming/Balatro/1/save.jkr")
            backup_dir = os.path.expanduser("~/.steam/steam/steamapps/compatdata/2379780/pfx/drive_c/users/steamuser/AppData/Roaming/Balatro/1/autosave")
        ...
        return save_file_path, backup_dir

    def perform_backup(self):
        """Perform the backup task"""
        try:
            # Define paths
            save_file_path, backup_dir = self.get_save_paths()
    ...
    def play_game(self):
        ...
        try:
            ...
            if os.path.exists(game_executable):
                if platform.system() == "Linux":
                    # Run the game using Steam and Proton GE on Linux with WINEDLLOVERRIDES
                    steam_command = [
                        "steam",  # Ensure that 'steam' is in your PATH, or provide the full path
                        "steam://run/2379780",  # Replace '2379780' with the actual App ID of the game if different
                        f"--proton-ge",  # Use the specific Proton GE version
                        f"-- WINEDLLOVERRIDES=version=n,b %command%"  # Set the WINEDLLOVERRIDES environment variable
                    ]
                    print(f"Launching game with Steam and Proton GE: {steam_command}")
                    subprocess.run(steam_command, check=True)
                else:
                    # Launch the game executable on other platforms
...
############################################################
    ...
    def install_mods(self, popup):
        ...
        if os.path.isdir(self.mods_dir):
            ...
            # Ask the user if they want to backup the current Mods folder
            backup_msg_box = QMessageBox()
            backup_msg_box.setIcon(QMessageBox.Icon.Question)
            backup_msg_box.setWindowTitle("Backup Mods Folder")
            backup_msg_box.setText("Do you want to backup the current 'Mods' folder?")
            backup_msg_box.setStandardButtons(QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
            backup_response = backup_msg_box.exec()

            if backup_response == QMessageBox.StandardButton.Yes:
                # Create backup of current Mods folder
...
>>>>>>> 382a4e613cd7215bfa244258f5559bcdfbb0e191
